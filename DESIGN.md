...
2. 搜索网络资源（如 GitHub、MCP 市场、公共 API 文档、SKILL 市场）——使用已有的搜索工具和 `fetch` 工具。
...
**工具集**：与 Use-Tool Agent 一致，从主循环 `delegate` 的 `tools` 参数中获取，不做特殊处理。主循环在调用 Find-Tools Agent 时，应在 `tools` 参数中包含搜索工具、注册工具和 `execute_bash`。
...
| `fetch` | builtin | function | 发送 HTTP 请求，获取网页内容或调用 API |
...
| `baidu-search` | default | skill | 百度搜索的封装 skill，注册时 `inject_to_system_prompt: false`，仅作为可调用工具使用 |

至此，最小集合工具箱包含了上下文管理工具（`context_summarizer`）、永久记忆工具（`memory-recall`）、本地搜索工具（`local-search`）、文件操作工具（`filesystem`）、网络搜索工具（`serper-search` + `baidu-search`）、用户交互工具（`ask_user`）以及完整的工具注册体系（支持 http/code/skill/cli/mcp 五种类型）。其中 skill 工具支持两种注入模式，灵活满足不同场景。
...
1. config.json有重复的嵌入模型配置，已优化为顶层 `embedding` 字段，memory 和 tool_registry 共用
2. 使用openai mode, completion or response, 已支持：`api_mode` 字段配置，`LLMClient.chat()` 自动分派，`format_tool_result()` 兼容两种格式
3. 没有推送成功：https://github.com/juzcn/qd-agents
