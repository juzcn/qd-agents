...
**MCP工具展开：**
- 在上下文管理中，MCP工具会自动连接到MCP服务器并展开所有可用工具
- 将MCP服务器提供的每个工具作为独立工具加载到可用工具列表中
- 大模型在使用tool calling时可以获得完整的工具信息
- 更新调度器为工具调用模式，支持MCP工具自动展开
...
**架构简化：**
- 两阶段调度简化为单阶段工具调用，提高响应速度
- 工具执行器重构为模块化结构，支持插件化扩展
- 添加 BASH 工具类型，支持直接执行 bash 命令

**功能增强：**
- 集成 Skill 工具支持，支持渐进式披露的复杂工作流
- 增强工具注册中心，支持更多工具执行类型
- 优化模型选择算法，改进 fallback 机制
...
**优势**：
- **简化架构**：减少调度层级，降低系统复杂度
- **提高性能**：单次 LLM 调用完成工具选择，减少延迟
- **易于维护**：模块化工具执行器支持插件化扩展
- **灵活适配**：支持 tool-use 和 code-plan 两种工作模式切换
...
**简化架构**：当前实现采用单阶段工具调用模型，LLM 直接访问完整的工具集进行意图理解和工具选择，提高了响应速度和系统可维护性。

**工作原理**：
1. LLM 接收用户输入、会话历史和完整的可用工具列表
2. 智能体根据用户意图选择合适的处理方式：
   - **direct**：直接生成自然语言回复，不调用任何工具
   - **find_tools**：检索与查询相关的工具列表
   - **普通工具**：直接调用已注册的特定工具
   - **coding_tool_use**：生成 Python 代码编排复杂逻辑（条件分支、循环等）
   - **step_down**：降级处理，返回用户友好的提示
3. 执行引擎根据工具类型调用相应的执行器完成实际操作
...
- **系统简洁**：消除两阶段调度的复杂性，降低维护成本
- **扩展性强**：模块化执行器支持新工具类型，易于功能扩展
- **可观测性好**：统一接口和清晰的职责分离，便于调试和监控
...
- **性能优化**：单次 LLM 调用减少延迟，提高用户体验
- **架构简化**：消除两阶段调度的复杂性和状态管理
- **工具集规模可控**：当前工具集规模适中，单次 prompt 可容纳所有工具定义
- **维护性提升**：统一调度逻辑降低系统复杂度和测试负担

**单阶段架构优势**：
- **响应速度快**：端到端延迟显著降低
- **系统简洁**：减少代码复杂性和潜在错误点
- **调试方便**：单次调用链路更易于追踪和调试
- **成本优化**：减少 LLM 调用次数，降低使用成本
...
## 18. 技能转 MCP 系统 (skill2mcp)

### 18.1 设计目标

将技能封装为 Python MCP 服务，使用大模型理解分析 SKILL.md 内容，将技能能力封装为 MCP 服务，供大模型调用，解决技能语义逻辑的不确定性问题。

- **本质**：一个 coding agent，按照开放规范迭代完成 MCP 服务的开发，全自动进行
- **简单技能**：作为 MCP wrapper，直接调用原始技能脚本
- **复杂技能**：涉及逻辑编排，根据技能复杂度自动适配生成策略
- **依赖**：仅依赖 SKILL.md 提供的能力描述和参数定义

### 18.2 核心功能

1. **智能技能分析**：
   - 使用 LLM 深度分析技能目录，提取精确参数类型和约束
   - 支持复杂参数类型（list[obj]、obj 等）到 JSON Schema 的映射
   - 自动识别技能复杂度（simple/complex）和技能类型（command_line/api_service/script/composite）

2. **模板化代码生成**：
   - 基于 Jinja2 模板系统生成 MCP 服务器，避免硬编码解决方案
   - 模板与代码分离，易于维护和扩展
   - 支持技能目录中的多种脚本类型（Python、Shell、JavaScript）

3. **自动注册与集成**：
   - 生成 MCP 服务器后自动注册到工具注册表
   - 支持命令行选项 `--register` 控制注册行为
   - 与现有 MCP 服务器管理功能无缝集成

4. **自我迭代优化**：
   - coding agent 风格，持续优化生成的代码质量
   - 复用项目现有的提示词模板管理系统
   - 根据生成结果反馈优化后续生成策略

### 18.3 技术实现

- **模板引擎**：Jinja2（项目已依赖，功能强大，支持复杂模板逻辑）
- **参数映射**：智能参数类型映射，支持复杂参数到 Python/JSON Schema 的转换
- **配置管理**：使用项目现有的配置系统，支持环境变量和配置文件
- **错误处理**：完善的错误处理机制，确保生成代码的可靠性
