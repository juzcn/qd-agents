...
class ToolUseModeOrchestrator:
    ...
    async def orchestrate(
        ...
        # 始终使用单阶段工具调用
        result = await self._run_tool_use(result, user_input, expanded_tools, history)
    ...
    async def _run_tool_use(
        ...
        if choice.message.tool_calls:
            ...
            # 设置 OpenAI tool calling 标准字段
            result.tool_call_id = tool_call.id
            result.tool_name = tool_call.function.name
            result.tool_input = phase_two_result.tool_input
...
# 向后兼容性：保持 TwoPhaseOrchestrator 别名
TwoPhaseOrchestrator = ToolUseModeOrchestrator
