...
async def _chat_async(
    ...
    # 自动启动 MCP 天气服务器（如果未运行）
    async def auto_start_mcp_weather_server():
        """自动启动 MCP 天气服务器（如果未运行）"""
        ...
        async def start_mcp_server():
            ...
            # 获取跨平台可执行文件路径（在 try 块外部，以便错误处理中使用）
            ...
            try:
                console.print("[dim]正在检查 MCP 天气服务器状态...[/]", style="dim")

                # 检查端口 8000 是否可用
                if not check_port_available(8000):
                    console.print("[dim]✅ MCP 天气服务器已在运行 (端口 8000)[/]", style="dim")
                    return None

                console.print("[dim]正在启动 MCP 天气服务器...[/]", style="dim")

                # 启动 mcp-weather-server 进程
                # 使用 subprocess.Popen 以便在会话结束时清理
                # 使用 exe 文件路径，为未来集成一般 CLI 工具做准备
                mcp_server_process = subprocess.Popen(
                    [str(exe_path), "--mode", "sse", "--port", "8000"],
                ...
                for i in range(10):
                    ...
                    if not check_port_available(8000):
                        console.print("[dim]✅ MCP 天气服务器启动成功[/]", style="dim")
                ...
                console.print("[yellow]⚠️  MCP 天气服务器启动超时，将继续使用演示模式[/]", style="dim")
                return None

            except FileNotFoundError:
                console.print(f"[yellow]⚠️  未找到 {exe_name} 可执行文件[/]", style="dim")
                console.print("[dim]  请安装: uv add mcp-weather-server[/]", style="dim")
                return None
            except Exception as e:
                console.print(f"[yellow]⚠️  启动 MCP 服务器失败: {e}[/]", style="dim")
                console.print("[dim]  将继续使用演示模式[/]", style="dim")
                return None
        ...
        def cleanup():
            ...
            if mcp_server_process and mcp_server_process.poll() is None:
                try:
                    console.print("[dim]正在停止 MCP 天气服务器...[/]", style="dim")
                    mcp_server_process.terminate()
                    mcp_server_process.wait(timeout=2)
                    console.print("[dim]✅ MCP 天气服务器已停止[/]", style="dim")
                except Exception as e:
                    console.print(f"[dim]停止 MCP 服务器时出错: {e}[/]", style="dim")
                    try:
        ...
        return cleanup if server_process else None

    # 启动 MCP 天气服务器
    cleanup_mcp_server = await auto_start_mcp_weather_server()
    ...
    # 自动检查并注册 MCP 天气工具（如果不存在）
    async def auto_register_mcp_tools():
    ...
    async def init_llm_and_agent(p: str, m: str | None = None):
